...
import { Crown, Lock, DollarSign, ArrowLeft, Home } from 'lucide-react';
...
const CollectCashPage: React.FC = () => {
    ...
    const togglePayment = async (id: string, currentStatus: boolean) => {
        try {
            await setDoc(doc(db, "settings", "team_payment_status"), {
                [id]: !currentStatus
            }, { merge: true });
        } catch (error) {
        ...
        <div className="min-h-screen bg-[#020817] text-slate-100 font-sans selection:bg-sky-500/30 overflow-x-hidden">
            ...
            <main className="relative max-w-7xl mx-auto px-6 py-32 space-y-32">
                ...
                <header className="text-center space-y-8 max-w-4xl mx-auto">
                    ...
                    >
                        Collect <br />
                        <span className="text-transparent bg-clip-text bg-gradient-to-r from-sky-400 via-indigo-400 to-purple-400">Cash Portal</span>
                    </motion.h1>
                    ...
                    >
                        Manage team member visibility. Enable payments to reveal identity.
                    </motion.p>
                </header>
                ...
                >
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        <TeamMember
                            realName="Jeevasurya"
                            realRole="Team Lead & Full Stack"
                            id="24506375"
                            initials="JS"
                            color="amber"
                            isLead={true}
                            isPaid={!!paymentStatus["24506375"]}
                            onToggle={() => togglePayment("24506375", !!paymentStatus["24506375"])}
                        />
                        <TeamMember
                            realName="Gokul S"
                            realRole="Developer"
                            ...
                            onToggle={() => togglePayment("24506364", !!paymentStatus["24506364"])}
                        />
                        <TeamMember
                            realName="Gokulsanjay V"
                            realRole="Developer"
                            id="24506365"
                            initials="GV"
                            color="violet"
                            ...
                            onToggle={() => togglePayment("24506365", !!paymentStatus["24506365"])}
                        />
                        <TeamMember
                            realName="Gokulraj M"
                            realRole="Developer"
                            ...
                            onToggle={() => togglePayment("24506366", !!paymentStatus["24506366"])}
                        />
                    </div>
                </motion.section>
...
const TeamMember = ({ realName, realRole, id, initials, color, isLead = false, isPaid, onToggle }: any) => {
    ...
    const colorMap: any = {
        ...
        amber: "from-amber-400 to-orange-600 shadow-amber-500/30 text-amber-400"
    };
    ...
    const displayName = isPaid ? realName : "Amount Not Paid";
    const displayRole = isPaid ? realRole : "Pay 980 Rupees to Enable name";
