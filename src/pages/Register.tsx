...
import { Eye, EyeOff, Mail, Lock, Loader2, AlertCircle, User as UserIcon, ArrowLeft } from 'lucide-react';
...
const InteractiveRegisterIcon = () => {
  ...
  return (
    <div ref={ref} className="w-full h-24 flex items-center justify-center mb-4 cursor-grab perspective-1000">
      <motion.div
        style={{ rotateX, rotateY, transformStyle: "preserve-3d" }}
        className="relative w-20 h-20 md:w-24 md:h-24"
      >
        <motion.div
          className="absolute inset-0 bg-indigo-500/30 dark:bg-indigo-500/20 rounded-full blur-3xl transform translate-y-4"
          style={{ x: shadowX, y: shadowY }}
        />
        <svg viewBox="0 0 100 100" className="absolute inset-0 w-full h-full drop-shadow-2xl" style={{ transform: 'translateZ(25px)' }}>
          <defs>
...
export default function Register() {
  ...
  const handleGoogleRegister = async () => {
    // Google Register does not require pre-checking terms, as they are redirected to /accept-terms
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await setDoc(doc(db, 'users', userCredential.user.uid), {
        ...
        termsAccepted: true, // Manual registration verified via checkbox
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 dark:bg-[#020817] text-slate-900 dark:text-slate-50 font-sans p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          <Card className="bg-white/70 dark:bg-[#0B1121]/70 backdrop-blur-xl border border-slate-200/60 dark:border-slate-800/60 rounded-3xl shadow-2xl shadow-indigo-900/5">
            <CardHeader className="text-center p-8 pb-4">
              <InteractiveRegisterIcon />
              <CardTitle className="text-2xl font-bold font-sora mb-2">Create Account</CardTitle>
              <CardDescription className="text-slate-500 dark:text-slate-400">Start your free trial today</CardDescription>
            </CardHeader>
            <CardContent className="p-8 pt-0 space-y-5">
              ...
              <div className="relative my-2">
                ...
                <div className="relative flex justify-center text-xs uppercase"><span className="bg-white dark:bg-[#0F1629] px-3 py-1 rounded text-slate-400">Or register with email</span></div>
              </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative group">
                    ...
                    <Input
                      ...
                      className={`pl-11 h-12 bg-slate-50 dark:bg-slate-900/50 border-slate-200 dark:border-slate-800 rounded-xl focus:ring-2 focus:ring-sky-500/20 ${errors.name ? 'border-red-500' : ''}`}
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative group">
                    ...
                    <Input
                      ...
                      className={`pl-11 h-12 bg-slate-50 dark:bg-slate-900/50 border-slate-200 dark:border-slate-800 rounded-xl focus:ring-2 focus:ring-sky-500/20 ${errors.email ? 'border-red-500' : ''}`}
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative group">
                    ...
                    <Input
                      ...
                      className={`pl-11 pr-11 h-12 bg-slate-50 dark:bg-slate-900/50 border-slate-200 dark:border-slate-800 rounded-xl focus:ring-2 focus:ring-sky-500/20 ${errors.password ? 'border-red-500' : ''}`}
                      placeholder="••••••••"
                      disabled={loadingState !== 'idle'}
                    />
                ...
                <div className="space-y-2 px-1">
                  <div className="flex items-start space-x-3">
                    <input
                      ...
                      className="mt-1 peer h-4 w-4 shrink-0 rounded border border-slate-300 dark:border-slate-600 focus:ring-2 focus:ring-sky-500 checked:bg-sky-600 checked:border-sky-600 cursor-pointer"
                    />
                    <Label htmlFor="terms" className="text-sm font-normal text-slate-600 dark:text-slate-400 leading-tight cursor-pointer">
                      I agree to the <Link to="/terms" className="text-sky-600 font-medium hover:underline">Terms of Service</Link> and <Link to="/privacy" className="text-sky-600 font-medium hover:underline">Privacy Policy</Link>.
                    </Label>
                  </div>
                  {errors.terms && <p className="text-sm text-red-500 ml-8 mt-1">{errors.terms}</p>}
                </div>

                <Button type="submit" size="lg" className="w-full h-12 rounded-full bg-gradient-to-r from-sky-500 to-indigo-600 text-white font-bold shadow-xl shadow-sky-500/20 hover:scale-[1.02] active:scale-[0.98] transition-all" disabled={loadingState !== 'idle'}>
            ...
            <CardFooter className="p-6 justify-center bg-slate-50 dark:bg-slate-900/30 border-t border-slate-200/50 dark:border-slate-800/50 rounded-b-3xl">
              <p className="text-sm text-slate-600 dark:text-slate-400">Already have an account? <Link to="/login" className="font-bold text-sky-600 dark:text-sky-400 hover:underline">Sign in</Link></p>
            </CardFooter>
          </Card>
        </motion.div>
